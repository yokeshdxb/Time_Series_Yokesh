...
# Basic page config
st.set_page_config(page_title="Yokesh Gold Price Forecasting", layout="wide")

# Title and description
st.title("Yokesh Gold Price Forecasting App")
st.write("This app forecasts gold prices(USD) using time series models (ARIMA/SARIMA).")

# Sidebar inputs
with st.sidebar:
    st.header("Settings")
    model_choice = st.selectbox("Select Model", ["arima_gold_model_2000_2025.pkl", "sarima_gold_model_2000_2025.pkl"])
    forecast_months = st.slider("Forecast Period (months)", 1,12)
...
def load_gold_data():
    ...
    gold_data = yf.download("GC=F", start="2000-01-01", end=end_date, interval="1mo")
